...
% Command használata az RLS-hez és a Cargo-hoz
% stdin() stdout() RLS esetén
% serde-json
% - struktúrák készítése a lehetséges RLS válaszok / értesítésekhez
% - LSP-n utánanézni, mik ezek
% Cargo new bin/lib, cargo init bin/lib, cargo build, cargo build --release, cargo check, cargo test, cargo bench, cargo doc (--open)